...
function updateStatistics() {
    ...
    const storageSize = new Blob([JSON.stringify({ tabs, trash: trashData })]).size;
    ...
    document.getElementById('statLastVisit').textContent = lastVisit ? formatRelativeTime(parseInt(lastVisit)) : 'Never';
}
...
function updateAllSbaces() {
    ...
    if (sortedTabs.length === 0 && searchQuery) {
        container.innerHTML = `
            <div style="text-align:center;padding:40px;color:var(--text-muted);">
                <div style="font-size:17px;font-weight:600;margin-bottom:6px;">No sbaces found</div>
                <div style="font-size:13px;">Try adjusting your search terms</div>
            </div>
        `;
    ...
    if (sortedTabs.length === 0 && !searchQuery) {
        container.innerHTML = `
            <div style="text-align:center;padding:40px;color:var(--text-muted);">
                <div style="font-size:17px;font-weight:600;margin-bottom:6px;">No sbaces yet</div>
                <div style="font-size:13px;">Create your first sbace to get started</div>
            </div>
        `;
        showMoreBtn.style.display = 'none';
        return;
    ...
    if (searchQuery) {
        showMoreBtn.style.display = 'none';
    } else if (sortedTabs.length > allSbacesVisible && !showingAll) {
...
function searchSbaces(query) {
    ...
    if (searchQuery === '') {
        ...
        Object.entries(tabs).forEach(([id, tab]) => {
            ...
            if (nameMatch || contentMatch) filteredTabs[id] = tab;
        });
    }
...
function initializeEventListeners() {
    ...
    document.getElementById('quickFindInput').addEventListener('keydown', function(e) {
        ...
        else if (e.key === 'Escape') { hideQuickFind(); }
    });

    document.getElementById('quickFindInput').addEventListener('click', e => e.stopPropagation());
    document.getElementById('quickFindOverlay').addEventListener('click', e => e.stopPropagation());
    ...
    document.addEventListener('keydown', function(e) {
        if (e.key === ' ') checkAutoFormat(e);

        if (e.ctrlKey && e.key === 'f') { e.preventDefault(); showQuickFind(); return; }

        if (e.ctrlKey && e.key === 'z' && !e.shiftKey) { e.preventDefault(); undoAction(); }
        else if ((e.ctrlKey && e.shiftKey && e.key === 'Z') || (e.ctrlKey && e.key === 'y')) { e.preventDefault(); redoAction(); }
        ...
        if (e.key === 'Enter') {
            ...
            if (spaceModal.classList.contains('show')) { e.preventDefault(); confirmSpaceName(); }
        }
    });
    ...
    editor.addEventListener('keydown', function(e) {
        ...
        if (e.key === 'Tab') {
            ...
            const node = selection.anchorNode;
            let listItem = node;
            while (listItem && listItem !== editor && listItem.nodeName !== 'LI') listItem = listItem.parentNode;

            if (listItem && listItem.nodeName === 'LI') {
                const currentLevel = getListLevel(listItem);
                if (e.shiftKey) {
                    if (currentLevel > 0) document.execCommand('outdent');
                } else {
                    const range = selection.getRangeAt(0);
                    const curListItem = range.startContainer.parentElement.closest('li');
                    if (curListItem) {
                        const savedSelection = { startOffset: range.startOffset, startContainer: range.startContainer };
                        ...
                        newParentList.style.listStyleType = newLevel % 2 === 1 ? 'lower-alpha' : 'decimal';
                        const newRange = document.createRange();
    ...
    editor.addEventListener('keyup', function(e) {
        if (e.key === ' ' || e.key === 'Enter' || e.key === 'Tab') setTimeout(forceSpellCheck, 0);
    });
    ...
    editor.addEventListener('paste', function(e) {
        ...
        if (htmlContent) {
            ...
            const walker = document.createTreeWalker(tempDiv, NodeFilter.SHOW_ELEMENT, {
                acceptNode: node => allowedTags.includes(node.tagName) ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_REJECT
            });
            const nodesToRemove = [];
            let node;
            while (node = walker.nextNode()) {
                if (!allowedTags.includes(node.tagName)) nodesToRemove.push(node);
            }
    ...
    document.addEventListener('click', function(e) {
        if (!e.target.closest('.nav-options') && !e.target.closest('.options-bar')) hideOptionsBar();
    });

    ['deleteModal', 'clearModal', 'spaceNameModal', 'findReplaceModal', 'trashModal', 'importModal'].forEach(id => {
        document.getElementById(id).addEventListener('click', function(e) {
            if (e.target === this) {
                const fn = {
                    deleteModal: hideDeleteModal,
                    clearModal: hideClearModal,
                    spaceNameModal: hideSpaceNameModal,
                    findReplaceModal: hideFindReplaceModal,
                    trashModal: hideTrashModal,
                    importModal: hideImportModal
                }[id];
                if (fn) fn();
            }
        });
